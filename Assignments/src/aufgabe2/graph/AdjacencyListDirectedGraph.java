...
public class AdjacencyListDirectedGraph<V> implements DirectedGraph<V> {
	...
	@Override
	public boolean addVertex(V v) { // punkt
		if (v == null)
			throw new NullPointerException("Knoten darf nicht null sein.");
    ...
    public boolean addEdge(V v, V w, double weight) {
		if (v == null || w == null)
			throw new NullPointerException("Knoten dürfen nicht null sein");
		if (v.equals(w))
			throw new IllegalArgumentException("(v == w) nicht erlaubt.");

		addVertex(v);
		addVertex(w);

		boolean isNewEdge = !succ.get(v).containsKey(w); // prüfen, ob Kante neu
    ...
    public double getWeight(V v, V w) {
		if (!containsEdge(v, w))
			throw new IllegalArgumentException("Kante "+v+ " → " + w + " existiert nicht.");
    ...
    public int getInDegree(V v) {
		if (!containsVertex(v))
			throw new IllegalArgumentException("Knoten " + v + " nicht im Graph enthalten.");
	...
	DirectedGraph<V> invert() {
		DirectedGraph<V> inv = new AdjacencyListDirectedGraph<>(); // neue Instanz
		for (V v : succ.keySet()) {
			for (Map.Entry<V, Double> entry : succ.get(v).entrySet()) { // iteriere über Nachfolger
				V w = entry.getKey();
				double weight = entry.getValue();
				inv.addEdge(w, v, weight);
			}
		}
		return inv;
	}
	...
	public String toString() {
		...
		for (V v : succ.keySet()) {
			for (Map.Entry<V, Double> entry : succ.get(v).entrySet()) {
				sb.append(v)
						.append(" --> ")
						.append(entry.getKey())
						.append(" weight = ")
	...
	public static void main(String[] args) {
		...
		// es handelt sich um eine nicht modifizierbare Sicht auf die Menge,
		// kann aufgrund der Kapselung nicht von außen verändert werden
	}
}
