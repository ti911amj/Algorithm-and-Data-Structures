    ...
    public DepthFirstOrder(DirectedGraph<V> g) {
        ...
        for (V v : g.getVertexSet()) { // geht durch alle Knoten
            if (!visited.contains(v)) {
                numberOfDFTrees++; // neuer Baum
                dfs(v); // starte Tiefensuche
            }
        }
    }
    // rekursive DFS-Methode
    private void dfs(V v) {
        visited.add(v);  // besucht
        preOrder.add(v);  // Sofort zur PreOrder hinzufügen

        for (V w : myGraph.getSuccessorVertexSet(v)) { // geht durch alle Nachfolger
            if (!visited.contains(w)) {
                dfs(w);  // Rekursiv Nachfolger besuchen
            }
        }

        postOrder.add(v); // Am Ende zur PostOrder hinzufügen
    }
